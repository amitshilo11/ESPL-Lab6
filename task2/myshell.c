...
int cmp_arg00(cmdLine* cmd_line, char* str) {
    if(cmd_line->arguments[0][0] == '!')
        return cmd_line->arguments[0][1] - '0';
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    while(1) {
       ...
       // printf("%s",history_list[step]);
        ...
        else if(cmp_arg00(curr_line, "!") != 0) {
            int index = cmp_arg00(curr_line, "!");
            //printf("%d",index);
            freeCmdLines(curr_line); 
            //printf("%s",history_list[index]);
            curr_line = parseCmdLines(history_list[index]);
            execute(curr_line);
        }
        else {
            execute(curr_line);
